...
import AgentModelSelector from '../Menus/Endpoints/AgentModelSelector';
...
const ChatForm = memo(({ index = 0, isAgent = false }: { index?: number; isAgent?: boolean }) => {
  ...
  const hasNoAgentid = useMemo(() => {
    if (location.search.includes('agent_id')) {
      return false;
    } else {
      return true;
    }
              ...
              {true && (
                <>
                  <div style={{ height: 0, width: 0, overflow: 'hidden' }}>
                    <AgentModelSelector startupConfig={startupConfig} />
                  </div>
