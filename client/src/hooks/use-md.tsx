...
import { Children, Fragment, createElement, isValidElement, useEffect, useMemo, useState, useRef, JSX } from "react";
...
export const ANCHOR_CLASS_NAME =
  "font-semibold underline underline-offset-[2px] decoration-1 hover:text-[#0563B2] transition-colors";
      ...
      .use(rehypeReact, {
        ...
        components: {
          ...
          h1: ({ children, id }: JSX.IntrinsicElements["h1"]) => (
            <h1 className=" font-semibold text-2xl mb-6 mt-6" id={id}>
              {children}
            </h1>
          ),
          h2: ({ children, id }: JSX.IntrinsicElements["h2"]) => (
            <h2 className=" font-medium text-2xl  mb-6 mt-6" id={id}>
              {children}
            </h2>
          ),
          h3: ({ children, id }: JSX.IntrinsicElements["h3"]) => (
            <h3 className=" font-semibold text-xl  mb-6 mt-2" id={id}>
              {children}
            </h3>
          ),
          h4: ({ children, id }: JSX.IntrinsicElements["h4"]) => (
            <h4 className=" font-medium text-xl  my-6" id={id}>
              {children}
            </h4>
          ),
          h5: ({ children, id }: JSX.IntrinsicElements["h5"]) => (
            <h5 className=" font-semibold text-lg  my-6" id={id}>
              {children}
            </h5>
          ),
          h6: ({ children, id }: JSX.IntrinsicElements["h6"]) => (
            <h6 className=" font-medium text-lg  my-6" id={id}>
              {children}
            </h6>
          ),
          p: (props: JSX.IntrinsicElements["p"]) => {
            return (
              <p className="text-xs mb-6" style={{ lineHeight: "24px" }}>
                {props.children}
              </p>
            );
          },
          ...
          pre: ({ children }: JSX.IntrinsicElements["pre"]) => {
            return (
              <div className="relative mb-6">
                <pre className="p-4 rounded-lg border-1 border-[#E0E0E0] bg-[#F4F4F5] [&>code.hljs]:p-0 [&>code.hljs]:bg-transparent font-code text-xs overflow-x-auto flex items-start">
                  {children}
                </pre>
              </div>
            );
          ...
          ol: ({ children }: JSX.IntrinsicElements["ol"]) => (
            <ol className="flex flex-col gap-3  my-6 pl-3 [&_ol]:my-3 [&_ul]:my-3">
              {Children.map(flattenChildren(children).filter(isValidElement), (child, index) => (
                <li key={index} className="flex gap-2 items-start">
                  <div className=" text-xs  font-semibold shrink-0 min-w-[1.4ch]" aria-hidden>
                    {index + 1}.
                  </div>
                  {child}
                </li>
          ...
          li: ({ children }: JSX.IntrinsicElements["li"]) => <div className=" text-xs">{children}</div>,
          table: ({ children }: JSX.IntrinsicElements["table"]) => (
            <div className="overflow-x-auto mb-6">
              <table className="table-auto border-1 border-[#E0E0E0]">{children}</table>
            </div>
          ),
          thead: ({ children }: JSX.IntrinsicElements["thead"]) => <thead className="bg-[#E4E4E5]">{children}</thead>,
          th: ({ children }: JSX.IntrinsicElements["th"]) => (
            <th className="border-1 border-[#E0E0E0] p-2  text-xs font-semibold ">{children}</th>
          ),
          td: ({ children }: JSX.IntrinsicElements["td"]) => (
            <td className="border-1 border-[#E0E0E0] p-2  text-xs">{children}</td>
          ),
          blockquote: ({ children }: JSX.IntrinsicElements["blockquote"]) => (
            <blockquote className="border-l-4 border-[#E0E0E0] pl-2 italic">{children}</blockquote>
          ),
        },
      })
...
const CodeBlock = ({ children, className }: JSX.IntrinsicElements["code"]) => {
    ...
    return (
      <>
        ...
        <div className="flex flex-col gap-1 flex-grow-0 flex-shrink-0">
          {/* <button
            type="button"
            className="rounded-md p-1  hover:bg-[#E4E4E5] border-1 border-[#E0E0E0] transition-colors"
          ...
          >
            {copied ? <div className="w-4 h-4">核对</div> : <div className="w-4 h-4">复制</div>}
          </button> */}
          {/* {isMermaid ? (
            <>
              <button
          ...
          {isLatex ? (
            <>
              ...
              {/* <Dialog open={showLatexPreview} setOpen={setShowLatexPreview} title="Latex diagram preview" size="3xl">
  ...
  return <code className="inline-block font-code bg-[#E4E4E5] p-0.5  -my-0.5">{children}</code>;
};
...
//         <p className=" text-xs text-slate-700">Rendering diagram...</p>
//       </div>
//     );
//   } else if (diagram === false) {
//     return <p className=" text-xs text-slate-700">Unable to render this diagram.</p>;
//   } else {
  ...
  if (diagram === true) {
    ...
    return (
      <p className=" text-xs text-slate-700">
        ...
        <a href="https://mermaid.live/edit" className={ANCHOR_CLASS_NAME} target="_blank">
          Mermaid Live Editor
        </a>
        .
      </p>
