...
class ApiService {
  ...
  async getSessionDetails(sessionId: string): Promise<SessionMetadata> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.SESSION_DETAILS}?sessionId=${sessionId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
  ...
  async getSessionEvents(sessionId: string): Promise<AgentEventStream.Event[]> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.SESSION_EVENTS}?sessionId=${sessionId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
        signal: AbortSignal.timeout(5000), // 添加 5 秒超时
      });
  ...
  async getSessionStatus(sessionId: string): Promise<{ isProcessing: boolean; state: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.SESSION_STATUS}?sessionId=${sessionId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
  ...
  async mockSendStreamingQuery(
    ...
    try {
      // 从 public 目录里读取
      const res = await fetch(`/mocks/${query}.txt`);
      const mockTxt = await res.text();

      // 拆分成多个事件块（用两个换行分隔）
      const events = mockTxt.trim().split("\n\n");

      for (const eventString of events) {
        if (eventString.startsWith("data: ")) {
          try {
            const eventData = JSON.parse(eventString.substring(6));
            console.log("mock event:", eventData);
            onEvent(eventData);

            // 模拟 SSE 逐条推送
            await new Promise((res) => setTimeout(res, 50));
          } catch (e) {
            console.error("解析事件失败:", e, eventString);
          }
        }
      }
    } catch (err) {
      console.error("读取 mock txt 出错:", err);
    }
  }
  ...
  async sendStreamingQuery(
    ...
    if (
      query == "生成中港铝板项目任务持续时间和项目费用图表" ||
      query == "金利锋轮毂是否满足轮毂设计精度要求" ||
      query === "帮我写一份周报"
    ) {
      await this.mockSendStreamingQuery(sessionId, query, onEvent, thinking, userId);
    } else {
      try {
        const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.QUERY_STREAM}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sessionId, query, userId: userId, thinking: thinking ? "1" : undefined }),
        });
  ...
  async getBrowserControlInfo(sessionId: string): Promise<{ mode: string; tools: string[] }> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/sessions/browser-control?sessionId=${sessionId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
        signal: AbortSignal.timeout(3000),
      });
