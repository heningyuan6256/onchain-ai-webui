...
import { TabDataRender } from "./TabDataRender";
 ...
 */
export const ToolResultRenderer: React.FC<ToolResultRendererProps> = ({ content, onAction, className = "" }) => {
  if (!content || content.length === 0) {
    return <div className="p-4 text-gray-500 dark:text-gray-400 text-sm italic">No content to display</div>;
  }

  return (
    <div className={`space-y-4 ${className} h-full`}>
      {content.map((part, index) => {
        
        part.tool = part.tool || "";
        if (part.tool.indexOf("onchain_rag_query") != -1) {
          return <RagQueryRenderer part={part} />;
        }

        if (part.tool.indexOf("onchain_tab_") != -1) {
          return <TabDataRender part={part} />;
        }

        console.log(part,'part.tool');
        
        if (part.tool.indexOf("onchain_search_") != -1) {
          return <TabDataRender part={part} />;
        }

        if (part.type === "json") {
          return (
            <div key={`json-${part.name || ""}-${index}`} className="tool-result-part h-full">
        ...
        return (
          <div key={`${part.type}-${part.name || ""}-${index}`} className="tool-result-part h-full">
