...
export const Message: React.FC<MessageProps> = ({
  ...
  const getProseClasses = () => {
    if (message.role === 'user') {
      return 'prose prose-invert prose-sm max-w-none text-xs';
    } else {
      // For helper messages, use normal prose but apply prose-invert in dark mode
      return 'prose dark:prose-invert prose-sm max-w-none text-xs';
    }
  };

  console.log(isFinalAssistantResponse && hasEnvironmentState, 'isFinalAssistantResponse && hasEnvironmentState');

  return (
    <div>
      <div className={`message-container ${message.role === 'user' ? 'message-container-user' : 'message-container-assistant'}`}>
