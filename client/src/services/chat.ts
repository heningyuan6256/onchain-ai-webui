...
export const chatService = {
	...
	async fetchMessages(chatId: any, api_key: string): Promise<ChatMessage[]> {
		const res = await fetch(`${API_URL}?chatId=${!chatId ? '' : encodeURIComponent(chatId)}&user=${api_key}`);
	...
	async getAppTimes(workflowid: string) {
		const res = await fetch(`/workflow/system/workflow/app_usenum?workflowid=${workflowid}`, {
			method: 'get',
			headers: { 'Content-Type': 'application/json' },
		});
		if (!res.ok) throw new Error('Failed to update favorite');
	...
	async getIsupload(workflowid: string) {
		const res = await fetch(`/workflow/system/workflow/query?workflow_id=${workflowid}`, {
			method: 'get',
			headers: { 'Content-Type': 'application/json' },
		});
		if (!res.ok) throw new Error('Failed to update favorite');
