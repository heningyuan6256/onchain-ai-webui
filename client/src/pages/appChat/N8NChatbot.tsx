...
const N8NChatbot: React.FC = () => {
	...
	useEffect(() => {
		...
		fetch(`/apps/system/app/query_app?app_id=${appId}`, requestOptions).then((res) => {
			res.json().then((data) => {
				setApp(data?.rows);
					...
					if (res?.code === 200) {
						setIsupload(
							res?.data?.data?.nodes?.find(
								(node: any) =>
									node.type === '@n8n/n8n-nodes-langchain.chatTrigger' && node.parameters?.options?.allowFileUploads === true
							) !== undefined
						);
					}
				});
	...
	const { chatSessions, currentSessionId, messages, createNewSession, switchSession, deleteSession, updateMessages, saveCurrentSession } =
		useChatSessions();

	const { sendMessage, isLoading } = useChat(messages, updateMessages, currentSessionId, n8nService);
		...
		<div className='app-shell'>
			...
			<div className='content-wrapper'>
				<div className='main-area'>
					<ChatMessages onSubmit={sendMessage} app={app} messages={messages} isLoading={isLoading} />
					<div style={{ paddingInline: '25%' }}>
						<ChatTextBox isupload={isupload} onSubmit={sendMessage} disabled={isLoading} onSetKnowledge={() => {}} knowledge={false} />
					</div>
				</div>
				<ChatSidebar title={app.app_name} interactionParams={appCount} appDescription={app.app_description} />
			</div>
		</div>
	);
};
