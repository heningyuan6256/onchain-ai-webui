...
export default function ResultPanel({ result, loading, imagePreview, onCopy, onDownload }) {
	...
	return (
		<div className='glass p-6 rounded-2xl space-y-4 h-full' style={{ backgroundColor: '#ffff', color: 'black', border: '1px solid #e0e0e0' }}>
			<div className='flex items-center justify-between'>
				<div className='flex items-center gap-2'>
					<Sparkles className='w-5 h-5 text-400' />
			...
			<AnimatePresence mode='wait'>
				{loading ? (
					...
					>
						...
						<p className='text-sm  animate-pulse'>AI正在解析你的图片</p>
					</motion.div>
				) : result ? (
					<motion.div
						key='result'
					...
					>
						...
						<div className='bg-white/5 border border-white/10 rounded-xl p-4 max-h-96 overflow-y-auto'>
							{isHTML ? (
								<div className='prose prose-invert prose-sm max-w-none' dangerouslySetInnerHTML={{ __html: result.text }} />
							) : isMarkdown ? (
						...
						{result.raw_text && (
							<details className='glass rounded-xl overflow-hidden'>
								...
								<div className='px-4 py-3 border-t border-white/10 space-y-2'>
									<p className='text-xs  mb-2'>模型的未处理输出（可用于调试）</p>
									<div className='bg-black/30 rounded-lg p-3 max-h-64 overflow-y-auto'>
										<pre className='text-xs  font-mono whitespace-pre-wrap break-words select-all'>{result.raw_text}</pre>
									</div>
									<div className='flex gap-2 mt-2'>
										...
										<span className='text-xs  py-1'>{result.raw_text.length} 字符</span>
									</div>
								</div>
							</details>
						)}
						...
						<details className='glass rounded-xl overflow-hidden'>
							...
							<div className='px-4 py-3 border-t border-white/10 space-y-3'>
								{result.metadata && (
									<div>
										<p className='text-xs  mb-2'>处理元数据</p>
										<pre className='text-xs  whitespace-pre-wrap'>{JSON.stringify(result.metadata, null, 2)}</pre>
									</div>
								)}
								{result.boxes?.length > 0 && (
									<div>
										<p className='text-xs  mb-2'>解析的边界框 ({result.boxes.length})</p>
										...
										<p className='text-xs  mt-2'>坐标从模型输出（0-999）缩放到图像像素</p>
									</div>
								)}
							</div>
						</details>
					...
					>
						<div className='relative'>
							...
							/>
							<Sparkles className='w-10 h-10 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-400' />
						</div>
						<div className='text-center'>
							<p className='text-lg font-medium '>准备就绪</p>
							<p className='text-sm  mt-1'>上传一张图片，点击分析！</p>
						</div>
					</motion.div>
				)}
			</AnimatePresence>
